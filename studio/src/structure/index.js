...
export const structure = (S) => S.list()
    ...
    S.listItem()
        .title('Shop 商品')
        .icon(DocumentIcon)
        .child(S.documentTypeList('shopProduct')
        .title('Shop 商品')
        .defaultOrdering([{ field: 'orderIndex', direction: 'asc' }])),
    S.listItem()
        .title('Rooms 空间')
        .child(S.documentTypeList('room')
        .title('Rooms 空间')
        .defaultOrdering([{ field: 'orderIndex', direction: 'asc' }])),
    S.listItem()
        .title('Events 活动')
        ...
        S.divider(),
        ...years.map((year) => S.listItem()
            .title(`${year} 年`)
            .icon(CalendarIcon)
            .child(S.list()
            .title(`${year} 年`)
            .items(months.map((month) => S.listItem()
            .title(month.title)
            .child(S.documentTypeList('event')
            .title(`${month.title} ${year}`)
            .filter(`_type == "event" && date match "${year}-${month.value
            .toString()
            .padStart(2, '0')}*"`)
            .defaultOrdering([{ field: 'date', direction: 'asc' }])))))),
    ])),
    ...
    S.listItem()
        ...
        S.listItem()
            .title('Documents 文档')
            .icon(DocumentIcon)
            .child(S.documentTypeList('consentDocument')
            .title('Consent Documents 公约文档')
            .filter('_type == "consentDocument"')
            .defaultOrdering([{ field: 'publishedAt', direction: 'desc' }])
            .child((documentId) => S.list()
            .title('Document')
            .items([
            S.listItem()
                .title('Edit Document')
                .icon(DocumentIcon)
                .child(S.document().schemaType('consentDocument').documentId(documentId)),
            S.listItem()
                .title('Clauses 条款')
                .icon(ListIcon)
        ...
        S.listItem()
            ...
            .icon(ListIcon)
            .child(S.documentTypeList('consentClause')
