...
function block(key: string, text: string): SeedBlock {
  return {
    _key: key,
    _type: 'block',
    style: 'normal',
...
const shopProducts = [
  {
    ...
    description: {
      en: [
        block(
          'set-socks-en-1',
          'Classic .TAG socks with a clean white knit and embroidered SET mark. Price can be updated later once the final merch list is confirmed.',
        ),
      ],
      cn: [
        block(
          'set-socks-cn-1',
          '经典 .TAG 白色短袜，带有 SET 刺绣标记。价格可在最终商品清单确认后再更新。',
        ),
      ],
    },
    variants: [],
  ...
  },
  {
    ...
    description: {
      en: [
        block(
          'marathon-cap-en-1',
          'A washed black cap embroidered with the Chengdu Marathon 2020 mark. A placeholder image is used in the site until the final packshot is added.',
        ),
      ],
      cn: [
        block(
          'marathon-cap-cn-1',
          '水洗黑色帽款，绣有 Chengdu Marathon 2020 标记。当前站点使用占位图，正式产品图可之后替换。',
        ),
      ],
    },
    variants: [],
